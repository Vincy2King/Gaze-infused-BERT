...
class SpanClassifier(nn.Module):
    ...
    def forward(self, span_emb_1, span_emb_2, label=None):
        ...
        probs = self.output(similarity).view(-1)
        outputs = (similarity,)
        print(probs)
        print(label)
...
class BertForSpanClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, span_1, span_2, labels=None,
                nfixs=None,ffds=None,trts=None, gpts=None,gds=None):
        ...
        outputs = self.bert(input_ids, token_type_ids, attention_mask, head_mask=None,
                            ...
                            TRTs=trts,
                            nFixs=nfixs)
        # print('BertForSpanClassification')
    ...
    def _extract_span_emb(self, sequence_outputs, span):
        ...
        prod = sequence_outputs * span.unsqueeze(-1).float().to(sequence_outputs.device)
...
class BertForSequenceClassificationMultiTask(BertPreTrainedModel):
    ...
    def __init__(self, config, task_num_labels, tasks):
        ...
        self.classifiers = torch.nn.Linear(config.hidden_size,  3)
    ...
    def forward(self, task_id, input_ids, token_type_ids, attention_mask, labels=None,
        ...
        # print('BertForSequenceClassificationMultiTask')
        pooled_output = outputs[1]
        pooled_output = self.dropout(pooled_output)

        # classifier = self.classifiers[task_id]
        logits = self.classifiers(pooled_output)

        outputs = (logits,) + outputs[2:]
        num_labels = 3#self.task_num_labels[self.id2task[task_id]]
        # loss= 0
        # print('num label:',num_labels)
        # print('logits:',logits,logits.view(3,-1))
        if labels is not None:
            if num_labels == 1:
                ...
                loss = loss_fct(logits,labels)#(logits.view(-1, num_labels), labels.view(-1))
            outputs = (loss,) + outputs
        # print('here:',logits.view(-1, num_labels),labels.view(-1))
        # print('logits:', logits)
        # print('loss:', loss)
        # loss_fct = CrossEntropyLoss()
        # loss = loss_fct(logits.view(-1, num_labels), labels.view(-1))
        # print('logits:',logits.view(self.batch_size,-1))
        # print('labels:',labels,labels.view(-1))
        # loss = loss_fct(logits.view(self.batch_size,-1).squeeze(), labels)
...
class RobertaForSequenceClassificationMultiTask(BertPreTrainedModel):
    ...
    def forward(self, task_id, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        print('RobertaForSequenceClassificationMultiTask')
